...
import WinnersBracketRoundColumn from './roundColumn/WinnersBracketRoundColumn';
    ...
    const roundsMap = matches.reduce((acc, match) => {
      if (!acc[match.round]) {
        acc[match.round] = {
          ...
          round_name: match.round >= 98 ? 'Championship' : `Round ${match.round}`,
          matches: []
        };
      }
      ...
      <div ref={containerRef} className="overflow-x-auto hide-scrollbar">
        <div className="min-w-max">
          ...
          <div className="flex gap-28 px-7 mb-4">
            {rounds.map((round) => (
              <div key={`title-${round.round_number}`} className="w-72 text-lg font-medium text-center text-blue-500">
                {round.round_name}
              </div>
            ))}
          </div>
